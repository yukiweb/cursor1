...
mysupport.factory("msUser", ['msLang', function(msLang) {
    ...
    factory.hasPermission = function(id) {
        var permission = true;
        var functionIdList = [];
        if (MS.functions) {
            for (var i = 0; i < MS.functions.length; i++) {
                functionIdList.push(MS.functions[i]['resourceId']);
            }
        }
        if (angular.isArray(id) && id.length) {
            for (var i = 0; i < id.length; i++) {
                if (angular.isString(id[i]) && functionIdList.indexOf(id[i]) == -1) {
                    permission = false;
                    break;
                }
            }
        } else if (angular.isString(id) && functionIdList.indexOf(id) == -1) {
            permission = false;
        }
        return permission;
    };
