//angular模块初始化
mysupport = angular.module("mysupport", [
...
mysupport.decorator('uiViewDirective', DecoratorConstructor);
/**
...
angular.element(document).ready(function() {
	$.ajax({
		url: '/mysupport/user/getUserInfo',
		type: 'GET'
	}).success(function(resp) {
		$("#page-loading").hide();
		if(resp.errorCode == 'no_permission') {
			MS = {lang: 'zh'};
		}else {
			MS = resp;
		}
		angular.bootstrap(document, ['mysupport']);
	}).error(function() {
		$("#page-loading").hide();
		MS = {lang: 'zh'};
		angular.bootstrap(document, ['mysupport']);
