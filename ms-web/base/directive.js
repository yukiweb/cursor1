    ...
    return {
        ...
        link: function(scope, ele, attrs) {
            ...
            scope.sure = function(event) {
                scope.$emit('select', scope.appContent);
                angular.element(ele).css('display', 'none');
            }
        },
    ...
    return {
        ...
        link: function($scope) {
            ...
            $scope.getallVal = function() {
                ...
                $scope.$emit('selectDate', $scope.myDate);
            }
            //初始化
...
// 表单验证指令
mysupport.directive('msValid', function() {
    return {
        restrict: "EA",
        templateUrl: 'directive/validate/ms.valid.html',
        priority: 100,
        scope: {
            msValid: "=",
            patternTip: "@",
        ...
        controller: ['$scope', '$transclude', 'msLang', function($scope, $transclude, msLang) {

            $scope.patternMessage = $scope.patternTip || 'mysupport.error.pattern.message';
            $scope.requiredMessage = $scope.requiredTip || 'mysupport.error.required.message';
            $scope.maxMessage = $scope.maxTip || 'mysupport.error.max20.message';
            $scope.minMessage = $scope.minTip || "mysupport.error.min6.message";
            $scope.linkMessage = $scope.linkTip;
            $scope.uniqueMessage = $scope.uniqueTip;

            if($scope.form && ($scope.msValid.$name == $scope.form.$name)) {
                $scope.valid = $scope.form[$scope.name];
                console.log($scope.valid)
            } else {
                $scope.valid = $scope.msValid;
            }

            //判断是否需要给校验不通过的边框
            var validBorderFlag = $scope.validBorder || true;
            if(validBorderFlag) {
                $scope.$watch(function() {
                    return "" + $scope.valid.$valid + $scope.valid.$dirty + $scope.valid.$touched + $scope.valid.$$parentForm.submitted
                }, function() {
                    if(!$scope.valid.$valid && ($scope.valid.$touched || $scope.valid.$$parentForm.submitted)) {
                        var _ele = angular.element("[name=" + $scope.valid.$name + "]");
                        _ele.parent().addClass("w-invalid-border");
                        if(_ele.hasClass("scrollbar-dynamic")){
                            _ele.parent().parent().parent().addClass("w-invalid-border");
                        }               
                    } else {
                        var _ele = angular.element("[name=" + $scope.valid.$name + "]");
                        _ele.parent().removeClass("w-invalid-border");
                        if(_ele.hasClass("scrollbar-dynamic")){
                            _ele.parent().parent().parent().removeClass("w-invalid-border");
                        }                       
                    }
                }, true);
            }
        ...
        compile: function(element, attrs) {
            ...
            return {
                ...
                post: function(scope, element, attrs) {
                    var wInputDiv = angular.element(element).parent();
                    var inputElement = angular.element(element).parent().find("input")
                    if(inputElement.length == 0) {
                        inputElement = angular.element(element).parent().find("textarea");
                    }
                    inputElement.on("focus", function() {
                        wInputDiv.addClass("w-focus-border");
                        scope.$apply(function() {
                            scope.inputFocus = true;
                            delete scope.valid.$error.unique;
                        });

                    }).on("blur", function() {
                        wInputDiv.removeClass("w-focus-border");
                        scope.$apply(function() {
