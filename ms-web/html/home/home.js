...
mysupport.controller("home", ['$scope', 'msUser', 'msLang','$http', function($scope, msUser, msLang,$http) {
    $scope.showKpi = msUser.hasPermission(['queryuserlogintimes', 'querykpi']);
    ...
    $scope.kpiDraw = function() {
        ...
        mainR.style.width = ((document.documentElement.scrollWidth || document.body.scrollWidth) - 240) + "px";
    };
    $scope.drawCharts = function(group) {
        ...
        $http({
            url: 'mysupport/kpi/queryLoginTimes?group='+activeGroup,
            method: 'GET'
        }).then(function(data) {
        ...
        $http({
            method: 'GET',
            url: 'mysupport/kpi/query?group='+group
        }).then(
            function(data) {
                data = data.data;
                var accountcount = 0;
